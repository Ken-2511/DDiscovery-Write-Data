...
def open(device=None, config=0):
	...
	device_names = [("Analog Discovery", constants.devidDiscovery), ("Analog Discovery 2", constants.devidDiscovery2),
					...
					("Analog Discovery Pro 5250", constants.devidADP5250)]
...
def temperature(device_data):
	...
	if dwf.FDwfAnalogIOChannelNodeStatus(device_data.handle, ctypes.c_int(channel), ctypes.c_int(node),
										 ctypes.byref(temperature)) == 0:
		check_error()
...
def __get_info__(device_data):
	...
	if dwf.FDwfAnalogInChannelRangeInfo(device_data.handle, ctypes.byref(temp1), ctypes.byref(temp2),
										ctypes.byref(temp3)) == 0:
		check_error()
	...
	# offset information
	if dwf.FDwfAnalogInChannelOffsetInfo(device_data.handle, ctypes.byref(temp1), ctypes.byref(temp2),
										 ctypes.byref(temp3)) == 0:
		check_error()
	...
	for channel_index in range(device_data.analog.output.channel_count):
		...
		for node_index in range(device_data.analog.output.node_count[channel_index]):
			if dwf.FDwfAnalogOutNodeDataInfo(device_data.handle, ctypes.c_int(channel_index), ctypes.c_int(node_index),
											 0, ctypes.byref(temp1)) == 0:
				check_error()
		...
		temp2 = ctypes.c_double()
		for node_index in range(device_data.analog.output.node_count[channel_index]):
			if dwf.FDwfAnalogOutNodeAmplitudeInfo(device_data.handle, ctypes.c_int(channel_index),
												  ctypes.c_int(node_index), ctypes.byref(temp1),
												  ctypes.byref(temp2)) == 0:
				check_error()
		...
		templist2 = []
		for node_index in range(device_data.analog.output.node_count[channel_index]):
			if dwf.FDwfAnalogOutNodeOffsetInfo(device_data.handle, ctypes.c_int(channel_index),
											   ctypes.c_int(node_index), ctypes.byref(temp1), ctypes.byref(temp2)) == 0:
				check_error()
			templist1.append(temp1.value)
			templist2.append(temp2.value)
		...
		templist2 = []
		for node_index in range(device_data.analog.output.node_count[channel_index]):
			if dwf.FDwfAnalogOutNodeFrequencyInfo(device_data.handle, ctypes.c_int(channel_index),
												  ctypes.c_int(node_index), ctypes.byref(temp1),
												  ctypes.byref(temp2)) == 0:
				check_error()
			templist1.append(temp1.value)
	...
	for channel_index in range(device_data.analog.IO.channel_count):
		...
		for node_index in range(device_data.analog.IO.node_count[channel_index]):
			temp1 = ctypes.create_string_buffer(256)
			temp2 = ctypes.create_string_buffer(256)
			if dwf.FDwfAnalogIOChannelNodeName(device_data.handle, ctypes.c_int(channel_index),
											   ctypes.c_int(node_index), temp1, temp2) == 0:
				check_error()
		...
		temp3 = ctypes.c_int()
		for node_index in range(device_data.analog.IO.node_count[channel_index]):
			if dwf.FDwfAnalogIOChannelNodeSetInfo(device_data.handle, ctypes.c_int(channel_index),
												  ctypes.c_int(node_index), ctypes.byref(temp1), ctypes.byref(temp2),
												  ctypes.byref(temp3)) == 0:
				check_error()
			templist1.append(temp1.value)
			templist2.append(temp2.value)
		...
		templist3 = []
		for node_index in range(device_data.analog.IO.node_count[channel_index]):
			if dwf.FDwfAnalogIOChannelNodeStatusInfo(device_data.handle, ctypes.c_int(channel_index),
													 ctypes.c_int(node_index), ctypes.byref(temp1), ctypes.byref(temp2),
													 ctypes.byref(temp3)) == 0:
				check_error()
			templist1.append(temp1.value)
