...
def generate(device_data, channel, function, frequency, duty_cycle=50, data=[], wait=0, repeat=0, run_time=0,
			 idle=idle_state.initial, trigger_enabled=False, trigger_source=trigger_source.none,
	...
	if dwf.FDwfDigitalOutCounterInfo(device_data.handle, ctypes.c_int(channel), ctypes.c_int(0),
									 ctypes.byref(counter_limit)) == 0:
		check_error()
	...
	if function == constants.DwfDigitalOutTypePulse:
		...
		if dwf.FDwfDigitalOutCounterSet(device_data.handle, ctypes.c_int(channel), ctypes.c_int(low_steps),
										ctypes.c_int(high_steps)) == 0:
			check_error()
	...
	# load custom signal data
		...
		# load data
		if dwf.FDwfDigitalOutDataSet(device_data.handle, ctypes.c_int(channel), ctypes.byref(buffer),
									 ctypes.c_int(len(data))) == 0:
			check_error()
